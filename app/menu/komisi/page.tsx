...
import KomisiFormModal, {
  type KpiDefinitionItem,
  type KomisiFormMode,
  type KomisiFormState,
  type KpiJobItem,
  type KpiRankingItem,
...
const EMPTY_FORM: KomisiFormState = {
  mode: "TARGET_BULK",
...
export default function KomisiPage() {
  ...
  const [formMode, setFormMode] = useState<KomisiFormMode>("TARGET_BULK");
  const [form, setForm] = useState<KomisiFormState>(EMPTY_FORM);
  const [formError, setFormError] = useState("");

  const [selectedRanking, setSelectedRanking] = useState<KpiRankingItem | null>(
    null,
  );
  ...
  const openModal = (mode: KomisiFormMode) => {
    setFormMode(mode);
    setForm({
      ...EMPTY_FORM,
      mode,
        ...
        <div className="flex flex-col gap-4 border-b-2 border-[#C92C1E] p-5 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="mb-1 flex items-center gap-2 text-xs font-bold text-gray-500">
              ...
              <span className="text-[#C92C1E]">Komisi</span>
            </div>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {loading ? (
                ...
                filteredRanking.map((item, index) => (
                  <tr
                    key={`${item.sales_id || index}-${item.rank_position || index}`}
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      {item.period_month || periodMonth}/{item.period_year || periodYear}
                    </td>
                  </tr>
                ))
              )}
              ...
              <tbody className="divide-y divide-gray-100">
                {targets.length === 0 ? (
                  ...
                  targets.slice(0, 8).map((item, index) => (
                    <tr key={`${item.sales_id || index}-${item.metric_code || index}`}>
                      <td className="px-4 py-4 font-black text-gray-900">
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-4 text-center">
                        {item.is_active === false ? (
                          ...
                          <button
                            type="button"
                            onClick={() => void handleDeactivateDefinition(item)}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-2 text-lg font-black text-slate-950 md:text-xl">
                      {selectedRanking.sales_name || selectedRanking.name || "-"}
                    </h2>
