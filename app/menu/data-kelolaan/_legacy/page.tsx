// Referensi migrasi beku untuk FE-02 (versi mock/localStorage lama dari
// halaman Kelolaan Customer). Folder diberi prefix `_` (konvensi "private
// folder" Next.js) supaya tidak menjadi route yang bisa diakses publik.
// Bug tipe pra-eksisting (`row.outlets` dipakai tanpa dideklarasikan di
// interface lokal) sudah diperbaiki dengan menambah field yang hilang,
// bukan disupresi — proyek ini melarang `@ts-nocheck`
...
// CATATAN FE-01: Ini adalah versi LEGACY (mock/localStorage) dari halaman
// Kelolaan Customer, dipertahankan sebagai referensi migrasi FE-02. Halaman
// utama `/menu/data-kelolaan` kini membaca data ASLI dari backend. Berkas ini
// tidak tertaut di menu dan masih menulis ke localStorage — jangan dijadikan
// sumber kebenaran.
interface NasabahItem {
  ...
  projectBrand: string;
  outlet: string;
  // Field ini dipakai (bukan dideklarasikan) di kode aslinya untuk multi-outlet
  // per owner — bug tipe pra-eksisting, sudah ada sejak HEAD sebelum FE-01.
...
// CATATAN FE-01: file asli (sebelum dipindah ke legacy/) punya DUA deklarasi
// `isValidInternationalPhone` dengan nama sama (bug pra-eksisting, sudah ada
// di HEAD sebelum Sprint FE-01 — bukan regresi dari sprint ini). Next.js 16
// Turbopack menolak build karena redeclare `const` adalah SyntaxError. Karena
// file ini legacy/tidak ter-routing, versi kedua (validasi berbasis jumlah
// digit) dihapus dan yang pertama (format E.164 eksplisit) dipertahankan
// tanpa mengubah perilaku bisnisnya — hanya supaya file tetap valid secara
// sintaks sebagai referensi migrasi FE-02.
